---
name: code-review
description: "Review pull requests and diffs in rios0rios0 — the profile stats generator that fetches GitHub, GitLab and Azure DevOps statistics and renders SVG widgets — against the rios0rios0/guide standards, with extra weight on snapshot-history integrity, graceful platform failure, and SVG escaping. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `rios0rios0`

This repository is both the profile README and the Go program that generates it: it fetches statistics from three platforms, persists a daily snapshot to `stats_history.json` on the `stats` branch, and renders per-year SVG widgets. A bug here corrupts an append-only history that cannot be recovered from the APIs after the fact.
...
## What matters most in `rios0rios0`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **The snapshot history is append-only and irreplaceable.** `accumulateByYear()` rebuilds per-year views by scanning every snapshot, and the APIs cannot re-serve yesterday's numbers. A change that can drop, overwrite, or duplicate a snapshot is a Critical finding — and `recalculate` mode deliberately replaces a whole year, so its guard rails matter most.
- **Every platform fetcher fails gracefully.** `FetchGitHubStats`, `FetchGitLabStats`, and `FetchAzureDevOpsStats` run in parallel and a failing platform is skipped, not fatal. A change that lets one outage abort the run breaks the daily job.
- **The three run modes have different contracts** — `daily` (today only, reuses languages), `bootstrap` (full current year), `recalculate` (full target year, replaces snapshots). A change must state which modes it affects.
- **SVG renderers are pure functions** returning strings. Any value interpolated into SVG — a repository name, a language, a token count — must be XML-escaped; an unescaped `&` or `<` produces a widget that silently fails to render.
- **`updateReadmeYearSections` rewrites `README.md` in place**, inserting new year `<details>` blocks in descending order. Check that a re-run is idempotent and cannot duplicate a section.
- **Credentials come from environment variables only.** A token must never reach a log line, an SVG, or the committed history.
- **There is no `make lint` or `make sast` here** — only `test` and `generate`. Do not ask for a gate the Makefile does not define; `go fmt ./... && go vet ./...` is the local substitute.

### Commands a reviewer should be able to quote

```bash
make test                                       # go test -tags=unit
go test -tags=unit -v -count=1 -run TestName ./...
make generate                                   # needs platform credentials
go fmt ./... && go vet ./...
```
...
- All test files carry `//go:build unit`; the suite does not run without `-tags=unit`.
- Three files by concern: `fetchers_test.go`, `helpers_test.go`, `svg_generators_test.go`.
- `testify` assertions, `t.Parallel()` plus `t.Run()`, BDD comment blocks.
- A renderer change needs an assertion on the produced SVG string, not a smoke test that it is non-empty.
...
- The single-file `main.go` layout — the program is intentionally one package.
- The absence of `lint`/`sast` Makefile targets.
